import {rehydrate_shell_data} from './lib/shell_data'
...
export function main_or_shell({reset_instance = false} = {}) {
  // Rehydrate data and try to boot straight to main_app
  const {user, instance_config, personalised_instance_id} = rehydrate_shell_data()
  ...
  if (have_enough_to_boot_main({user, instance_config, personalised_instance_id})) {
    ...
    launch_shell_app({user, instance_config, personalised_instance_id}, reset_instance)
  }
